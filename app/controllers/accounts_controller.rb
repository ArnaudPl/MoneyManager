  ...
  def printAccountLatestTransactions(account_id)
    ...
    transactions.each do |transaction|
      if transaction.withdraw
        ...
        html += view_context.link_to '<i class="material-icons white-text">delete</i>'.html_safe, [transaction.account, transaction], method: :delete, data: { confirm: 'Are you sure?' }, class: "transaction-row"
        html += '</li>'
      else
        ...
        html += '<span class="col s3">' + number_to_currency(transaction.amount).to_s + '</span>'
        html += '<span class="col s2">' + l(transaction.created_at, format: :date_day_month) + '</span>'
        html += view_context.link_to '<i class="material-icons white-text normal">delete</i>'.html_safe, [transaction.account, transaction], method: :delete, data: { confirm: 'Are you sure?' }, class: "transaction-row"
        html += '</li>'
      end
    end
    if html == ""
